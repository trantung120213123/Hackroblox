-- KeySystemGUI (gốc) + nhiều hào quang đẹp + draggable (kéo bằng header)
-- Features:
--  - Giữ nguyên chức năng: close ✕, Get Key (copy link), Submit key = "kk123", thông báo
--  - Thêm halo lớn phía sau GUI (xoay + pulse)
--  - Thêm halo ring/sparkle quanh logo (orbit)
--  - Logo có hover scale
--  - Kéo GUI bằng header (draggable)
...
-- assets (có thể đổi ID nếu muốn)
local HALO_ASSET = "rbxassetid://4996891970"    -- big soft glow
local RING_ASSET = "rbxassetid://6035067836"    -- small sparkle / ring image
...
halo.ZIndex = 1
halo.Image = HALO_ASSET
halo.ImageTransparency = 0.6
halo.Size = UDim2.new(0, 520, 0, 520) -- will be updated to fit main
halo.Position = UDim2.new(0, 20 + 210, 0, 80 + 110)
...
logoInner.TextSize = 18
logoInner.TextColor3 = Color3.fromRGB(245,245,245)
logoInner.TextScaled = false
...
-- ===== halo positioning & adaptive sizing =====
local function updateHalo()
    ...
    local haloSize = math.clamp(maxSide * 1.8, 300, 900)
...
-- ===== sparkle orbit around logo =====
spawn(function()
    local angle = 0
    while screenGui.Parent do
        local logoAbs = logoFrame.AbsolutePosition
        local mainAbs = main.AbsolutePosition
        local logoSize = logoFrame.AbsoluteSize
        -- orbit radius
        ...
        pcall(function()
            ...
            sparkle.Rotation = (sparkle.Rotation + 8) % 360
        end)
        task.wait(0.03)
    end
end)

-- ===== halo rotation + pulse tween loop =====
spawn(function()
    while screenGui.Parent do
        -- rotate slowly
        pcall(function() tween(halo, {Rotation = (halo.Rotation + 90) % 360}, 6, "Linear"):Play() end)
        -- pulse (slight scale up/down)
...
-- ===== logo glow (faint ring using UIGradient + UIStroke) =====
...
logoGlow.AnchorPoint = Vector2.new(0,0)
logoGlow.BackgroundTransparency = 1
logoGlow.Image = HALO_ASSET
logoGlow.ImageTransparency = 0.85
logoGlow.ZIndex = 2
logoGlow.ScaleType = Enum.ScaleType.Slice
logoGlow.SliceCenter = Rect.new(10,10,118,118)

-- gentle rotate + fade for logoGlow
spawn(function()
    while screenGui.Parent do
        pcall(function() tween(logoGlow, {Rotation = logoGlow.Rotation + 60}, 4, "Sine"):Play() end)
        pcall(function() tween(logoGlow, {ImageTransparency = 0.7}, 2.2, "Sine"):Play() end)
        task.wait(2.2)
        pcall(function() tween(logoGlow, {ImageTransparency = 0.9}, 2.2, "Sine"):Play() end)
        task.wait(2.2)
    end
end)
...
logoFrame.MouseEnter:Connect(function()
    pcall(function()
        tween(logoFrame, {Size = UDim2.new(0,48,0,48)}, 0.12, "Quad"):Play()
        tween(logoInner, {TextSize = 20}, 0.12):Play()
    end)
end)
logoFrame.MouseLeave:Connect(function()
    pcall(function()
        tween(logoFrame, {Size = UDim2.new(0,44,0,44)}, 0.12, "Quad"):Play()
        tween(logoInner, {TextSize = 18}, 0.12):Play()
    end)
end)
...
local function submitKey()
    ...
    if entered == VALID_KEY then
        ...
        task.delay(0.6, function() pcall(function() screenGui:Destroy() end) end)
    else
...
print("[KeySystemGUI] Loaded — added halo + draggable + logo glow.")

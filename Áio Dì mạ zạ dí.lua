...
local TP_TRASHCAN_INTERVAL = 0.01        -- chỉnh tốc độ hướng mặt
...
local function createCustomNotification(title, message, duration, color)
    duration = duration or 3
    color = color or Color3.fromRGB(70, 170, 230)
    
    local notificationGui = Instance.new("ScreenGui")
    notificationGui.Name = "KKNotification"
    ...
    titleLabel.Text = title or WINDOW_NAME
...
local function notify(t, m, sec)
    ...
    createCustomNotification(t or WINDOW_NAME, m or "", sec or 3)
end
...
local function performCycleFor(targetName)
    ...
    if not targetP then return false end
    ...
    if not trashA then return false end
    ...
    if cfA then 
        ...
        while tick() - t0 < TP_WAIT_AFTER_TRASH do
            ...
            task.wait(TP_TRASHCAN_INTERVAL) -- Cập nhật hướng nhìn mỗi 0.01 giây
        end
    end
...
-- Auto-kill all toggle and loop
local autoKillAll = false
local killAllLoopActive = false
local currentKillAllTarget = nil

-- Function to check if a player is dead (no character or humanoid health <= 0)
...
local function getRandomPlayer()
    ...
    -- If we have a current target and they're still alive, keep targeting them
    if currentKillAllTarget and Players:FindFirstChild(currentKillAllTarget) then
        local targetPlayer = Players:FindFirstChild(currentKillAllTarget)
        if not isPlayerDead(targetPlayer) then
            return currentKillAllTarget
        else
            notify(WINDOW_NAME, currentKillAllTarget .. " has been eliminated!", 3, Color3.fromRGB(220, 80, 80))
        end
    end
    ...
    for _, playerName in ipairs(players) do
        ...
        if player and not isPlayerDead(player) then
...
-- Auto Kill All button
local killAllBtn = Instance.new("TextButton", toggleGui)
killAllBtn.Name = "KillAllToggle"
killAllBtn.Size = UDim2.new(0, 160, 0, 40)
killAllBtn.Position = UDim2.new(0, 20, 0, 70)
killAllBtn.Text = "START KILL ALL"
killAllBtn.Font = Enum.Font.GothamBold
killAllBtn.TextSize = 16
killAllBtn.BackgroundColor3 = Color3.fromRGB(180, 60, 60)
...
-- Toggle functionality for single target
toggleBtn.MouseButton1Click:Connect(function()
            ...
            spawn(function()
                while autoKill do
                    if not selectedPlayer or selectedPlayer == "" then
                        ...
                        -- Kiểm tra xem target có còn tồn tại không
                        local targetExists = Players:FindFirstChild(selectedPlayer)
                        if not targetExists then
                            statusLabel.Text = "Status: Target left"
                            selectedPlayer = nil
                            notify(WINDOW_NAME, "Target left, stopping auto kill", 2)
                            autoKill = false
                            toggleBtn.Text = "START AUTO KILL"
                            toggleBtn.BackgroundColor3 = Color3.fromRGB(180, 60, 60)
                            break
                        ...
                        -- Thực hiện cycle và kiểm tra kết quả
                        local success = pcall(performCycleFor, selectedPlayer)
                        if not success then
                            statusLabel.Text = "Status: Error in cycle"
                            task.wait(0.2)
                        else
...
-- Toggle functionality for kill all
killAllBtn.MouseButton1Click:Connect(function()
    autoKillAll = not autoKillAll
    
    if autoKillAll then
        killAllBtn.Text = "STOP KILL ALL"
        killAllBtn.BackgroundColor3 = Color3.fromRGB(60, 180, 80)
        
        if not killAllLoopActive then
            killAllLoopActive = true
            spawn(function()
                while autoKillAll do
                    -- Lấy ngẫu nhiên một người chơi còn sống
                    ...
                    if not target then
                        ...
                        currentKillAllTarget = target
                        selectedPlayer = target
                        
                        statusLabel.Text = "Status: Hunting "..target
                        notify(WINDOW_NAME, "Now hunting: "..target, 2, Color3.fromRGB(220, 80, 80))
                        
                        -- Tiếp tục tấn công cho đến khi target chết
                        local targetPlayer = Players:FindFirstChild(target)
                        while autoKillAll and targetPlayer and not isPlayerDead(targetPlayer) do
                            -- Kiểm tra xem target có còn tồn tại không
                            ...
                            -- Thực hiện cycle và kiểm tra kết quả
                            local success = pcall(performCycleFor, target)
                            if not success then
                                statusLabel.Text = "Status: Error in cycle"
                                task.wait(0.2)
                            else
                                statusLabel.Text = "Status: Attacking "..target
                                task.wait(CYCLE_DELAY)
                            end
                            
                            -- Kiểm tra lại trạng thái của target
                            targetPlayer = Players:FindFirstChild(target)
                        end
                        
                        -- Thông báo nếu target đã chết
                        if targetPlayer and isPlayerDead(targetPlayer) then
                            notify(WINDOW_NAME, target .. " has been eliminated!", 3, Color3.fromRGB(220, 80, 80))
                        end
                    end
                end
                killAllLoopActive = false
...
-- Thêm event listener để xử lý khi target rời game
Players.PlayerRemoving:Connect(function(player)
    if selectedPlayer == player.Name and (autoKill or autoKillAll) then
        if autoKill then
            autoKill = false
            killLoopActive = false
        end
        
        if autoKillAll and currentKillAllTarget == player.Name then
            notify(WINDOW_NAME, player.Name.." left, finding new target", 2, Color3.fromRGB(220, 80, 80))
            currentKillAllTarget = nil
        end
...
-- Function to periodically check player status
spawn(function()
    while true do
        task.wait(1)
        
        -- Nếu đang trong chế độ kill all và target hiện tại chết, tìm target mới
        if autoKillAll and currentKillAllTarget then
            local targetPlayer = Players:FindFirstChild(currentKillAllTarget)
            if targetPlayer and isPlayerDead(targetPlayer) then
                notify(WINDOW_NAME, currentKillAllTarget .. " has been eliminated!", 3, Color3.fromRGB(220, 80, 80))
                currentKillAllTarget = nil

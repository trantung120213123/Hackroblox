...
-- ====== Thêm 2 nút: Trash Can V2 (chạy loadstring) + Auto TP Behind (toggle) ======
-- Paste chỗ sau khi tạo `toggleBtn` (trong 'content'), trước connect của toggleBtn

-- Run external Trashcan script button
local runTrashBtn = Instance.new("TextButton", content)
runTrashBtn.Size = UDim2.new(0,220,0,44)
runTrashBtn.AnchorPoint = Vector2.new(0,1)
runTrashBtn.Position = UDim2.new(0,16,1,-66) -- trên toggleBtn 50px
runTrashBtn.BackgroundColor3 = Color3.fromRGB(18,18,18)
runTrashBtn.BorderSizePixel = 0
runTrashBtn.Text = "Trash Can V2"
...
local runTrashCorner = Instance.new("UICorner", runTrashBtn); runTrashCorner.CornerRadius = UDim.new(0,8)
local runTrashStroke = Instance.new("UIStroke", runTrashBtn); runTrashStroke.Transparency = 0.8; runTrashStroke.Thickness = 1
addHover(runTrashBtn)

runTrashBtn.MouseButton1Click:Connect(function()
    pcall(function() 
        -- play click tween if muốn:
        tweenObject(runTrashBtn, {BackgroundTransparency = 0.2}, 0.08):Play()
        task.delay(0.08, function() tweenObject(runTrashBtn, {BackgroundTransparency = 0.45}, 0.08):Play() end)
    end)
    -- tải và chạy script từ repo
    pcall(function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/yes1nt/yes/refs/heads/main/Trashcan%20Man", true))()
    end)
end)

-- Auto TP Behind toggle button
local autoTPBtn = Instance.new("TextButton", content)
autoTPBtn.Size = UDim2.new(0,220,0,44)
autoTPBtn.AnchorPoint = Vector2.new(0,1)
autoTPBtn.Position = UDim2.new(0,16,1,-116) -- trên runTrashBtn thêm 50px
autoTPBtn.BackgroundColor3 = Color3.fromRGB(20,20,60)
autoTPBtn.BorderSizePixel = 0
autoTPBtn.Text = "Auto TP Behind: OFF"
autoTPBtn.Font = Enum.Font.Gotham
autoTPBtn.TextSize = 16
autoTPBtn.TextColor3 = Color3.fromRGB(230,230,230)
autoTPBtn.ZIndex = 3
local autoCorner = Instance.new("UICorner", autoTPBtn); autoCorner.CornerRadius = UDim.new(0,8)
local autoStroke = Instance.new("UIStroke", autoTPBtn); autoStroke.Transparency = 0.8; autoStroke.Thickness = 1
addHover(autoTPBtn)

local autoTPBehind = false
local AUTO_TP_INTERVAL = 0.01

local tpLoop = nil
autoTPBtn.MouseButton1Click:Connect(function()
    autoTPBehind = not autoTPBehind
    if autoTPBehind then
        autoTPBtn.BackgroundColor3 = Color3.fromRGB(18,120,18)
        autoTPBtn.Text = "Auto TP Behind: ON"
        -- start loop
        tpLoop = task.spawn(function()
            while autoTPBehind do
                local tgt = selectedTarget
                if tgt and tgt.Parent then
                    local tgtHrp = getHRP(tgt)
                    if tgtHrp then
                        local tpos = tgtHrp.Position
                        local behindPos = tpos - (tgtHrp.CFrame.LookVector * (BEHIND_STUDS or 5))
                        -- teleport player to behind (use existing tpTo)
                        pcall(function()
                            tpTo(CFrame.new(behindPos, tpos))
                        end)
                    else
                        status.Text = "Status: Target chưa spawn"
                    end
                else
                    status.Text = "Status: Chưa chọn target"
                end
        ...
        -- stop loop
        autoTPBehind = false
        -- tpLoop will stop on next condition; no need to explicitly cancel
    end
end)
-- ====== END added buttons ======
...
-- Logo design (match Tsb.lua)
local logoBg = Instance.new("Frame", smallBtn)
logoBg.Size = UDim2.new(1,0,1,0)
logoBg.Position = UDim2.new(0,0,0,0)
logoBg.BackgroundColor3 = Color3.fromRGB(30,30,30)
...
local kkLabel = Instance.new("TextLabel", logoBg)
...
local yellowDot = Instance.new("Frame", logoBg)
...
local halo = Instance.new("ImageLabel", smallBtn)
halo.Name = "HaloGlow"
halo.Size = UDim2.new(1.8,0,1.8,0)
halo.Position = UDim2.new(-0.4,0,-0.4,0)
halo.BackgroundTransparency = 1
halo.Image = "rbxassetid://4996891970"
...
clickSound.SoundId = SMALL_ASSET_CLICK
